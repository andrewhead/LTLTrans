...
public class LtlExplainer {
    ...
    public LtlExplainer(List<Object> propositions) {
        ...
        this.tempUnaries.add(NodeType.GLOBAL_NOT);
        ...
        if (o instanceof String || o instanceof Integer) {
            ...
            if (m.containsKey("binop")) {
                ...
                if (opType.equals("not")) {
                    ...
                    if (argType == NodeType.NOT) nt = NodeType.GLOBAL_NOT;
                    else if (argType == NodeType.FUTURE) nt = NodeType.GLOBAL_FUTURE;
    ...
    public NLGElement visit(Object n) {
        ...
        else if (type == NodeType.GLOBAL_NOT) result = this.visitGlobalNot(n);
    ...
    private NLGElement visitFuture(Object fn) {
        ...
        if (this.mDepth == 1) {
            ...
            this.addFrontModifier(child, "eventually");
            child.setFeature(Feature.TENSE, Tense.FUTURE);
        }
        return child;
    }

    private NLGElement visitGlobalNot(Object nfn) {
        Map m = (Map) nfn;
        Map nop = (Map) m.get("unop");
        Map arg = (Map) nop.get("arg");
        Map fop = (Map) arg.get("unop");
        NLGElement child = this.visit(fop.get("arg"));
        this.addFrontModifier(child, "it is never the case that");
        return child;
    }
