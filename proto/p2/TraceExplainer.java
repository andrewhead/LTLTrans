...
public class TraceExplainer {
    ...
    public TraceExplainer(Lexicon lexicon, NLGFactory nlgFactory, Realiser realiser) {
        this.mLexicon = lexicon;
        this.mNlgFactory = nlgFactory;
        this.mRealiser = realiser;
    }
    ...
    private SPhraseSpec makeLastClause(SourceType sType, ValueCount vc, ValueCount lastVc) {
        ...
        clause.setVerb(makeVerb(sType, true, lastVc.value, vc.value));
        clause.setObject(makeObject(sType, vc.value));
        clause = addCycleCount(clause, vc.count, "last");
        return clause;
    }
    ...
    public static void main(String [] args) {
        ...
        TraceExplainer explainer = new TraceExplainer(lexicon, nlgFactory, realiser);

        // Switch tests
        System.out.println(explainer.explainTrace(
            "the red light", SourceType.SWITCH, new int[] {0, 0, 0, 0, 0}));
        System.out.println(explainer.explainTrace(
            "the yellow light", SourceType.SWITCH, new int[] {1, 1, 1, 1, 1}));
        System.out.println(explainer.explainTrace(
            "the red light", SourceType.SWITCH, new int[] {0, 1, 1, 0, 0}));
        System.out.println(explainer.explainTrace(
            "the yellow light", SourceType.SWITCH, new int[] {0, 1, 0, 0, 1}));
        System.out.println(explainer.explainTrace(
            "the green light", SourceType.SWITCH, new int[] {1, 0, 1, 0, 0}));

        // Pure signal tests
        System.out.println(explainer.explainTrace(
            "the down signal", SourceType.PURE, new int[] {0, 1, 1, 1, 0}));
        System.out.println(explainer.explainTrace(
            "the up signal", SourceType.PURE, new int[] {0, 1, 1, 0, 0}));
        System.out.println(explainer.explainTrace(
            "the down signal", SourceType.PURE, new int[] {0, 1, 0, 0, 1}));

        // Integer tests
        System.out.println(explainer.explainTrace(
            "the counter", SourceType.INTEGER, new int[] {0, 0, 0, 1, 0}));
        System.out.println(explainer.explainTrace(
            "the counter", SourceType.INTEGER, new int[] {0, 0, 1, 1, 0}));
        System.out.println(explainer.explainTrace(
            "the counter", SourceType.INTEGER, new int[] {0, 1, 1, 1, 1}));
        System.out.println(explainer.explainTrace(
            "the counter", SourceType.INTEGER, new int[] {0, 0, 0, 0, 0}));
